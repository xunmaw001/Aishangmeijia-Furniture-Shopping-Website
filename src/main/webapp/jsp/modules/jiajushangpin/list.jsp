...
<body>
	<div id="main-container">
		...
		<div id="breadcrumb-container">
			<h3 class="breadcrumb-title">家具商品管理</h3>
			<ol class="breadcrumb-list">
				...
				<li class="breadcrumb-item-two"><span>家具商品列表</span></li>
			</ol>
		</div>
		<!-- /Breadcrumb -->
		...
		<div id="center-container">
			...
			<div class="center-form">
				<div class="form-item">
					<label>家具名称</label>
					<input type="text" id="jiajumingchengSearch" class="form-control form-control-sm" placeholder="请输入家具名称" aria-controls="tableId">
				</div>
			...
			<table id="tableId" class="table table-bordered">
				<thead>
					<tr>
						...
						<th onclick="sort('jiajubianhao')">家具编号<i id="jiajubianhaoIcon" class="fa fa-sort"></i></th>
						<th onclick="sort('jiajumingcheng')">家具名称<i id="jiajumingchengIcon" class="fa fa-sort"></i></th>
						<th>图片</th>
						<th onclick="sort('jiajuleixing')">家具类型<i id="jiajuleixingIcon" class="fa fa-sort"></i></th>
						<th onclick="sort('shengchanriqi')">生产日期<i id="shengchanriqiIcon" class="fa fa-sort"></i></th>
						<th onclick="sort('shiyongchanghe')">适用场合<i id="shiyongchangheIcon" class="fa fa-sort"></i></th>
						<th onclick="sort('jiajucaizhi')">家具材质<i id="jiajucaizhiIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function search(){
			searchForm = { key: ""};
			if($('#jiajumingchengSearch').val() != null && $('#jiajumingchengSearch').val() != ''){
				searchForm.jiajumingcheng ="%" +  $('#jiajumingchengSearch').val() + "%";
			}
        ...
        function getDataList() {
			http("jiajushangpin/page","GET",{
				...
				sort: sortColumn,
				order: sortOrder,
				jiajumingcheng : searchForm.jiajumingcheng,	
			},(res)=>{
				if(res.code == 0){
					clear();
		...
		function setDataRow(item,number){
			...
			var jiajubianhaoCell  = document.createElement('td');

						 jiajubianhaoCell.innerHTML = item.jiajubianhao;

			row.appendChild(jiajubianhaoCell);
			var jiajumingchengCell  = document.createElement('td');

						 jiajumingchengCell.innerHTML = item.jiajumingcheng;

			row.appendChild(jiajumingchengCell);
			...
			var jiajuleixingCell  = document.createElement('td');

						 jiajuleixingCell.innerHTML = item.jiajuleixing;

			row.appendChild(jiajuleixingCell);
			...
			var shiyongchangheCell  = document.createElement('td');

						 shiyongchangheCell.innerHTML = item.shiyongchanghe;

			row.appendChild(shiyongchangheCell);
			var jiajucaizhiCell  = document.createElement('td');

						 jiajucaizhiCell.innerHTML = item.jiajucaizhi;

			row.appendChild(jiajucaizhiCell);
			var jiajuxiangqingCell  = document.createElement('td');
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("jiajushangpin/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function discussList(id) {
            ...
            window.location.href = "../discussjiajushangpin/list.jsp"
        }
		//清除会重复渲染的节点
        function clear(){
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"jiajushangpin");
